...
object AppStrings {
    private val ui = mapOf(
        "pt" to mapOf(
            ...
            "choose_audio" to "Escolher áudio", "preview_result" to "▶ Pré-visualizar resultado", "save_error" to "Não foi possível guardar o resultado", "preview_selection" to "▶ Pré-visualizar seleção", "playing_selection" to "▶ A reproduzir seleção", "save_result" to "Guardar resultado", "saved_result" to "Resultado guardado", "reset_edit" to "↺ Repor seleção", "preview_ready" to "Pré-visualização pronta — ainda não guardado", "cut_action" to "Cortar e guardar", "cut_bad" to "Escolhe um intervalo válido dentro da duração do áudio.",
        ...
        "en" to mapOf(
            ...
            "choose_audio" to "Choose audio", "preview_result" to "▶ Preview result", "save_error" to "Could not save the result", "preview_selection" to "▶ Preview selection", "playing_selection" to "▶ Playing selection", "save_result" to "Save result", "saved_result" to "Result saved", "reset_edit" to "↺ Reset selection", "preview_ready" to "Preview ready — nothing saved yet", "cut_action" to "Cut and save", "cut_bad" to "Choose a valid interval inside the audio duration.", "cut_info" to "Drag the handles to set exactly where the cut starts and ends.",
        ...
        "fr" to mapOf(
            ...
            "choose_audio" to "Choisir un audio", "preview_result" to "▶ Prévisualiser le résultat", "save_error" to "Impossible d’enregistrer le résultat", "preview_selection" to "▶ Prévisualiser la sélection", "playing_selection" to "▶ Lecture de la sélection", "save_result" to "Enregistrer le résultat", "saved_result" to "Résultat enregistré", "reset_edit" to "↺ Réinitialiser la sélection", "preview_ready" to "Prévisualisation prête — rien n’est encore enregistré", "cut_action" to "Couper et enregistrer", "cut_bad" to "Choisis un intervalle valide dans la durée du fichier.", "cut_info" to "Fais glisser les poignées pour définir précisément le début et la fin.",
        ...
        "es" to mapOf(
            ...
            "choose_audio" to "Elegir audio", "preview_result" to "▶ Previsualizar resultado", "save_error" to "No se pudo guardar el resultado", "preview_selection" to "▶ Previsualizar selección", "playing_selection" to "▶ Reproduciendo selección", "save_result" to "Guardar resultado", "saved_result" to "Resultado guardado", "reset_edit" to "↺ Restablecer selección", "preview_ready" to "Vista previa lista — aún no se ha guardado", "cut_action" to "Cortar y guardar", "cut_bad" to "Elige un intervalo válido dentro de la duración del audio.", "cut_info" to "Arrastra los controles para definir exactamente el inicio y el final.",
        ...
        "de" to mapOf(
            ...
            "choose_audio" to "Audio auswählen", "preview_result" to "▶ Ergebnis vorhören", "save_error" to "Ergebnis konnte nicht gespeichert werden", "preview_selection" to "▶ Auswahl vorhören", "playing_selection" to "▶ Auswahl wird abgespielt", "save_result" to "Ergebnis speichern", "saved_result" to "Ergebnis gespeichert", "reset_edit" to "↺ Auswahl zurücksetzen", "preview_ready" to "Vorschau bereit — noch nichts gespeichert", "cut_action" to "Schneiden und speichern", "cut_bad" to "Wähle einen gültigen Bereich innerhalb der Audiodauer.", "cut_info" to "Ziehe die Griffe, um Anfang und Ende exakt festzulegen.",
    ...
    fun tool(context: Context, id: String): ToolCopy {
        return when (id) {
            ...
            "extract" -> {
                val lang = LanguageManager.get(context)
                val title = when (lang) { "en" -> "Extract audio"; "fr" -> "Extraire l’audio"; "es" -> "Extraer audio"; "de" -> "Audio extrahieren"; else -> "Extrair áudio" }
                val desc = when (lang) {
                    "en" -> "Extract a video's audio track to M4A locally."
                    "fr" -> "Extrait localement la piste audio d’une vidéo en M4A."
                    "es" -> "Extrae localmente la pista de audio de un vídeo en M4A."
                    "de" -> "Extrahiere die Audiospur eines Videos lokal als M4A."
                    else -> "Extrai localmente a faixa de áudio de um vídeo para M4A."
                }
                val steps = when (lang) {
                    "en" -> listOf("Choose a video", "Read its audio track", "Extract locally", "Save the M4A")
                    "fr" -> listOf("Choisis une vidéo", "Lis sa piste audio", "Extrait localement", "Enregistre le M4A")
                    "es" -> listOf("Elige un vídeo", "Lee su pista de audio", "Extrae localmente", "Guarda el M4A")
                    "de" -> listOf("Video auswählen", "Audiospur lesen", "Lokal extrahieren", "M4A speichern")
                    else -> listOf("Escolhe um vídeo", "Lê a faixa de áudio", "Extrai localmente", "Guarda o M4A")
                }
